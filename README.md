<div align="center">

<img src="https://capsule-render.vercel.app/api?type=soft&color=0A0E1A&height=200&section=header&text=Mayank%20Khandelwal&fontSize=48&fontColor=FF2BD6&desc=Full%20Stack%20Developer%20%7C%20Building%20Real-Time%20Systems%20for%20Intelligent%20Transportation&descSize=16&descAlignY=68&animation=fadeIn" width="100%"/>

<img src="https://readme-typing-svg.demolab.com/?font=Fira+Code&size=20&duration=2800&pause=1200&color=FF2BD6&center=true&vCenter=true&width=680&height=140&lines=%24+whoami;%3E+Mayank+Khandelwal+%E2%80%94+Full+Stack+Developer;%24+cat+currently_building.log;%3E+Real-time+Highway+Tolling+Dashboards;%24+echo+%24STACK;%3E+React.js+%2B+Node.js+%2B+Express.js+%2B+MongoDB;%24+status+--check;%3E+Open+to+SDE-1+%2F+Full+Stack+roles_" alt="Typing SVG" />

</div>

<div align="center">

![Degree](https://img.shields.io/badge/B.Tech-Computer%20Science%20%26%20Engineering-0A0E1A?style=flat-square&labelColor=FF2BD6)
![University](https://img.shields.io/badge/Chandigarh%20University-Mohali-0A0E1A?style=flat-square&labelColor=FF2BD6)
![Location](https://img.shields.io/badge/Location-Faridabad%2C%20Haryana%2C%20India-0A0E1A?style=flat-square&labelColor=FF2BD6)

</div>

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-FF2BD6?style=for-the-badge&logo=linkedin&logoColor=0A0E1A)](https://linkedin.com/in/kmayank7611)
[![Email](https://img.shields.io/badge/Email-FF2BD6?style=for-the-badge&logo=gmail&logoColor=0A0E1A)](mailto:mayankk1216@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-FF2BD6?style=for-the-badge&logo=github&logoColor=0A0E1A)](https://github.com/techiemankuu)
[![X](https://img.shields.io/badge/X%20%2F%20Twitter-FF2BD6?style=for-the-badge&logo=x&logoColor=0A0E1A)](https://x.com/MayankK1603)
[![Instagram](https://img.shields.io/badge/Instagram-FF2BD6?style=for-the-badge&logo=instagram&logoColor=0A0E1A)](https://instagram.com/mayaank_16)

</div>

<div align="center">

![Profile Views](https://komarev.com/ghpvc/?username=techiemankuu&color=FF2BD6&style=flat-square&label=PROFILE+VIEWS)
![Followers](https://img.shields.io/github/followers/techiemankuu?style=flat-square&color=FF2BD6&label=FOLLOWERS&logo=github&logoColor=0A0E1A&labelColor=0A0E1A)
![Stars](https://img.shields.io/badge/dynamic/json?style=flat-square&color=FF2BD6&label=REPOS&query=%24.length&url=https%3A%2F%2Fapi.github.com%2Fusers%2Ftechiemankuu%2Frepos&labelColor=0A0E1A)

</div>

---

### `$ cat about.md`

Performance-focused **Full Stack Developer** with ~1.5 years of hands-on experience building scalable, secure, production-ready applications. I specialize in backend architecture, REST API design, authentication systems, and database optimization — with a growing focus on **edge-computing systems that fuse real-world hardware telemetry into web dashboards**, most recently for highway tolling infrastructure. I care about clean architecture, secure-by-default APIs, and systems that hold up under real traffic — literal and figurative.

```bash
$ whoami
ROLE       : Full Stack Developer / Software Engineer
EXPERIENCE : ~1.5 Years
DOMAIN     : FinTech · Intelligent Transportation Systems · Smart Infrastructure
STACK      : React.js · Node.js · Express.js · MongoDB · MySQL
OPEN_TO    : SDE-1 · Full Stack Developer · Backend Engineer · MERN Developer · Platform Engineer
```

---

### `$ ls tech_stack/`

<div align="center">

![Skills](https://skillicons.dev/icons?i=js,java,cpp,html,css,react,nodejs&theme=dark)
<br/>
![Skills](https://skillicons.dev/icons?i=express,mongodb,mysql,git,github,postman,vscode&theme=dark)

</div>

<div align="center">

![REST APIs](https://img.shields.io/badge/REST-APIs-0A0E1A?style=flat-square&labelColor=FF2BD6)
![JWT Auth](https://img.shields.io/badge/JWT-Authentication-0A0E1A?style=flat-square&labelColor=FF2BD6)
![RBAC](https://img.shields.io/badge/RBAC-Access%20Control-0A0E1A?style=flat-square&labelColor=FF2BD6)
![Edge Computing](https://img.shields.io/badge/Edge-Computing-0A0E1A?style=flat-square&labelColor=FF2BD6)
![DSA](https://img.shields.io/badge/DSA-Problem%20Solving-0A0E1A?style=flat-square&labelColor=FF2BD6)
![Agile](https://img.shields.io/badge/Agile-Development-0A0E1A?style=flat-square&labelColor=FF2BD6)

</div>

---

### `$ cat expertise.table`

| Domain | Proficiency | Details |
|:--|:--:|:--|
| Backend Architecture | ⭐⭐⭐⭐⭐ | REST API design, Express.js service layers, modular MVC structure |
| Database Design & Optimization | ⭐⭐⭐⭐ | MongoDB indexing & aggregation, MySQL schema design, query tuning |
| Authentication & Security | ⭐⭐⭐⭐ | JWT-based auth, Role-Based Access Control (RBAC), secure API design |
| Real-Time & Edge Systems | ⭐⭐⭐⭐ | Fusing live telemetry (LiDAR, RFID, ANPR, AVC) into operational dashboards |
| Frontend Development | ⭐⭐⭐ | React.js component architecture, state management, responsive UI |
| Core CS Fundamentals | ⭐⭐⭐⭐ | DSA · OOP · DBMS · Operating Systems · Computer Networks |

---

### `$ ls featured_projects/ --sort=recent`

<details open>
<summary><b>🛣️ TrinitySoft.AI Gantry Dashboard</b></summary>
<br/>

A local edge tolling monitoring platform that aggregates telemetry streams from highway devices — LiDAR profilers, speed radars, ANPR cameras, RFID readers, and AVC systems — into unified vehicle transaction records.

| | |
|--|--|
| **Stack** | React.js · Node.js · Express.js · MongoDB · REST APIs |
| **Scale** | Multi-device real-time telemetry aggregation (LiDAR, RFID, ANPR, AVC, speed radar) |
| **Impact** | Enables toll operators to monitor lane activity, review vehicle classifications, detect overspeed events, and audit toll enforcement in real time |
| **My Contribution** | End-to-end full-stack development; real-time dashboards; async telemetry processing into structured vehicle passage records; lane monitoring, classification overrides, overspeed detection, enforcement auditing |

</details>

<details>
<summary><b>💰 Expense Management Platform</b></summary>
<br/>

A secure expense management platform with budgeting, transaction tracking, and optimized REST APIs.

| | |
|--|--|
| **Stack** | React.js · Node.js · Express.js · MongoDB · JWT |
| **Scale** | Full budgeting & transaction-tracking system with authenticated access |
| **Impact** | Secure, auth-gated expense and budget tracking for end users |
| **My Contribution** | Built JWT authentication, budgeting logic, and optimized REST API layer |

</details>

<details>
<summary><b>📦 Inventory Management System</b></summary>
<br/>

Backend system for inventory operations with authenticated, role-based CRUD APIs.

| | |
|--|--|
| **Stack** | Node.js · Express.js · MongoDB |
| **Scale** | Full CRUD API surface with RBAC-gated operations |
| **Impact** | Reliable, access-controlled inventory record management |
| **My Contribution** | Designed backend APIs, authentication, RBAC, and optimized MongoDB indexing |

</details>

<details>
<summary><b>🎬 Movie Recommendation System</b></summary>
<br/>

A recommendation engine built on movie metadata and user preferences.

| | |
|--|--|
| **Stack** | React.js · Node.js · TMDb API |
| **Scale** | TMDb-metadata-driven recommendation logic with caching |
| **Impact** | Faster response times via optimized API/caching strategy |
| **My Contribution** | Built recommendation engine and optimized TMDb API performance through caching |

</details>

---

### `$ tail -f experience.log`

**Full Stack Development Intern** · Rocket Science Technology  
📍 Faridabad, Haryana, India (On-site) · 🗓️ July 2026 – Present

- Developing enterprise-grade full-stack web applications using React.js, Node.js, Express.js, and MongoDB
- Building real-time monitoring dashboards and backend APIs for transportation and highway automation systems
- Integrating edge-device data, improving application performance, and collaborating with cross-functional engineering teams

`React.js` `Node.js` `Express.js` `MongoDB` `Edge Computing`

<br/>

**Software Development Intern** · Xpensease (Remote)  
🗓️ Jan 2026 – Jun 2026

- Engineered scalable full-stack applications using React.js, Node.js, Express.js, and MongoDB
- Designed and integrated RESTful APIs with JWT-based authentication, improving system efficiency by 30%
- Collaborated in Agile teams to deliver fintech and business solutions

`React.js` `Node.js` `JWT` `Agile` `FinTech`

<br/>

**Backend Engineer** · Zerobee Infotech (Remote)  
🗓️ Sep 2025 – Mar 2026

- Architected backend services and REST APIs using Node.js, Express.js, and MongoDB
- Improved application performance by 25% through MongoDB optimization
- Implemented authentication, authorization, and role-based access control

`Node.js` `Express.js` `MongoDB` `RBAC`

---

### `$ cat achievements.md`

<div align="center">

| Achievement | Context |
|:--|:--|
| 🚀 Improved system efficiency by **30%** | Designed & integrated JWT-authenticated REST APIs — *Xpensease* |
| ⚡ Improved application performance by **25%** | MongoDB query & indexing optimization — *Zerobee Infotech* |

</div>

---

### `$ whoami --education`

<div align="center">

![Degree](https://img.shields.io/badge/B.Tech-Computer%20Science%20%26%20Engineering-0A0E1A?style=for-the-badge&labelColor=FF2BD6)
![University](https://img.shields.io/badge/Chandigarh%20University-Mohali-0A0E1A?style=for-the-badge&labelColor=FF2BD6)

</div>

---

### `$ ls certifications/`

<div align="center">

![Blockchain](https://img.shields.io/badge/Infosys%20Springboard-Blockchain%20Technology-0A0E1A?style=flat-square&labelColor=FF2BD6)
![JavaScript](https://img.shields.io/badge/LinkedIn%20Learning-JavaScript%20Development-0A0E1A?style=flat-square&labelColor=FF2BD6)
![Python](https://img.shields.io/badge/LinkedIn%20Learning-Python%20Development-0A0E1A?style=flat-square&labelColor=FF2BD6)
![Web Dev](https://img.shields.io/badge/Coursera-Web%20Development-0A0E1A?style=flat-square&labelColor=FF2BD6)

</div>

---

### `$ ping coding_profiles`

<div align="center">

[![LeetCode](https://img.shields.io/badge/LeetCode-0A0E1A?style=for-the-badge&logo=leetcode&logoColor=FF2BD6)](https://leetcode.com/u/mayaank_16/)
[![HackerRank](https://img.shields.io/badge/HackerRank-0A0E1A?style=for-the-badge&logo=hackerrank&logoColor=FF2BD6)](https://www.hackerrank.com/profile/22BCS15197)
[![GeeksforGeeks](https://img.shields.io/badge/GeeksforGeeks-0A0E1A?style=for-the-badge&logo=geeksforgeeks&logoColor=FF2BD6)](https://www.geeksforgeeks.org/profile/mayankerx7)

</div>

---

### `$ top --github-analytics`

<div align="center">

<img height="165" src="https://github-readme-stats.vercel.app/api?username=techiemankuu&show_icons=true&theme=transparent&bg_color=0A0E1A&title_color=FF2BD6&icon_color=FF2BD6&text_color=00E5FF&border_color=6E7681&hide_border=false" />
<img height="165" src="https://github-readme-streak-stats.herokuapp.com/?user=techiemankuu&theme=transparent&background=0A0E1A&stroke=6E7681&ring=FF2BD6&fire=FF2BD6&currStreakLabel=FF2BD6&sideLabels=00E5FF&currStreakNum=00E5FF&sideNums=00E5FF&dates=6E7681&border=6E7681" />

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=techiemankuu&layout=compact&theme=transparent&bg_color=0A0E1A&title_color=FF2BD6&text_color=00E5FF&border_color=6E7681&hide_border=false" />

</div>

---

### `$ show --trophies --cols=7`

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=techiemankuu&theme=radical&column=7&row=2&margin-w=8&margin-h=8&no-frame=true&no-bg=false" />

</div>

---

### `$ plot activity_graph.svg`

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=techiemankuu&bg_color=0A0E1A&color=FF2BD6&line=FF2BD6&point=00E5FF&area=true&area_color=FF2BD6&hide_border=true" />

</div>

---

### `$ generate --summary-cards`

<div align="center">

<img width="100%" src="https://raw.githubusercontent.com/techiemankuu/techiemankuu/output/profile-details.svg" />

<img src="https://raw.githubusercontent.com/techiemankuu/techiemankuu/output/repos-per-language.svg" width="48%" />
<img src="https://raw.githubusercontent.com/techiemankuu/techiemankuu/output/most-commit-language.svg" width="48%" />

<img src="https://raw.githubusercontent.com/techiemankuu/techiemankuu/output/stats.svg" width="48%" />
<img src="https://raw.githubusercontent.com/techiemankuu/techiemankuu/output/productive-time.svg" width="48%" />

</div>

> These 5 cards render after the **Generate Summary Cards** workflow (included in this repo under `.github/workflows/`) runs at least once — trigger it manually from the Actions tab if it hasn't run yet.

---

### `$ ./snake.sh --render`

<div align="center">

<img src="https://raw.githubusercontent.com/techiemankuu/techiemankuu/output/github-contribution-grid-snake-dark.svg" />

</div>

> Rendered by the **Generate Snake** workflow (included under `.github/workflows/`) — runs every 6 hours, or trigger it manually the first time.

---

### `$ cat current_focus.yaml`

```yaml
current_focus:
  building: "TrinitySoft.AI Gantry Dashboard — real-time edge tolling monitoring for ITS"
  exploring: "Edge-computing patterns for fusing live hardware telemetry into web dashboards"
  open_to:
    - Software Engineer (SDE-1)
    - Full Stack Developer
    - Backend Engineer
    - MERN Stack Developer
    - Platform Engineer
  availability: "Remote / Hybrid / On-site — open to relocation"
```

---

### `$ ./connect.sh`

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-FF2BD6?style=for-the-badge&logo=linkedin&logoColor=0A0E1A)](https://linkedin.com/in/kmayank7611)
[![Email](https://img.shields.io/badge/Email-FF2BD6?style=for-the-badge&logo=gmail&logoColor=0A0E1A)](mailto:mayankk1216@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-FF2BD6?style=for-the-badge&logo=github&logoColor=0A0E1A)](https://github.com/techiemankuu)
[![X](https://img.shields.io/badge/X%20%2F%20Twitter-FF2BD6?style=for-the-badge&logo=x&logoColor=0A0E1A)](https://x.com/MayankK1603)
[![Instagram](https://img.shields.io/badge/Instagram-FF2BD6?style=for-the-badge&logo=instagram&logoColor=0A0E1A)](https://instagram.com/mayaank_16)

*"Ship it, measure it, harden it — then do it again."*

</div>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0A0E1A&height=120&section=footer&fontColor=FF2BD6" width="100%"/>
